...
define easyrsa::client (
  ...
  exec { "build-client-${title}":
    command   => "${easyrsa::install_dir}/easyrsa --pki-dir='${pki}' --keysize=${key_size} --batch --use-algo='${key_algo}' --days=${key_expire} --req-cn='${title}' --dn-mode=${dn_mode} --req-c='${country}' --req-st='${state}' --req-city='${city}' --req-org='${organization}' --req-ou='${org_unit}' build-client-full ${title} nopass",
